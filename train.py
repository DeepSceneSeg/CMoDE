...
def train_func(config):
    ...
    logits = model.build_graph(images_pl,images_pl1,labels_pl)
    ...
    print 'total_variables:',len(import_variables)        
    ...
    if ckpt and ckpt.model_checkpoint_path:
            ...
            step=int(ckpt.model_checkpoint_path.splt('/')[-1].split('-')[-1])+1
           ...
           for var in import_variables:
              if modality in var.name:  
                   ...
                   if temp[0]+' ' in name_var:
                        e[temp[0]]=var
           print 'varaibles_loaded:',len(e.keys())   
